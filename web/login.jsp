<%-- 
    Document   : login
    Created on : 14 janv. 2019, 11:32:30
    Author     : daryl
--%>

<%@include file="header.jsp"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>A R T K O D E S - Connexion</title>
    </head>
    
    <%@include file="alert.jsp" %>
    
    <body>
        <main role="main" class="container">
            <div class="col-lg-8 offset-lg-2">
                <h1>Connexion</h1>

                <form action="login" method="POST">
                    <fieldset>
                        <div class="form-group">
                            <label for="username">Nom d'utilisateur</label>
                            <input type="text" class="form-control" id="username" name="username" aria-describedby="emailHelp" placeholder="Nom d'utilisateur" data-kwimpalastatus="alive" data-kwimpalaid="1547462461565-3" required>
                            <span style="opacity: 1; left: 518px; top: 134.6px; width: 19px; min-width: 19px; height: 13px; position: absolute; background-image: url(&quot;data:image/svg+xml;base64,&quot;); background-repeat: no-repeat; background-position: 0px 0px; border: none; display: inline; visibility: visible; z-index: auto;"></span>
                        </div>

                        <div class="form-group">
                            <label for="password">Mot de passe</label>
                            <input type="password" class="form-control" id="password" name="password" placeholder="Mot de passe" data-kwimpalastatus="alive" data-kwimpalaid="1547462461565-4" required>
                            <span style="opacity: 1; left: 518px; top: 235.2px; width: 19px; min-width: 19px; height: 13px; position: absolute; background-image: url(&quot;data:image/svg+xml;base64,&quot;); background-repeat: no-repeat; background-position: 0px 0px; border: none; display: inline; visibility: visible; z-index: auto;"></span>
                        </div>
                        <button type="submit" class="btn btn-outline-primary">Se connecter</button>
                    </fieldset>
                </form>
            </div>
        </main> 
    </body>
    <%@include file="footer2.jsp" %>
</html>
